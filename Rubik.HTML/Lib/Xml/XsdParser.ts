module Rubik.Xml {
    ...
    function assert(condition: boolean, message?: any) {
    }
    ...
    function finaliseType(typeData) {
        if ((typeData.type == 'string') || (typeData.type == 'boolean') || (typeData.type == 'array') || (typeData.type == 'object')
            ...
            if (typeData.type.lastIndexOf('xml:', 0) === 0) { // id, lang, space, base, Father
    ...
    function doElement(src, parent, key) {
        ...
        if (name && type) {
            ...
            var minOccurs: number | string = 1;
            var maxOccurs: number | string = 1;
            var enumList = [];
            ...
            if (maxOccurs > 1) {
                var newTD: any = {};
                newTD.type = 'array';
                if (minOccurs > 0) newTD.minItems = parseInt(minOccurs.toString(), 10);
                if (maxOccurs < Number.MAX_VALUE) newTD.maxItems = parseInt(maxOccurs.toString(), 10);
    ...
    function moveProperties(obj, parent, key) {
        if (key == xsPrefix + 'sequence') {
            if (obj[key].properties) {
                ...
                var minOccurs: number | string = 1;
                var maxOccurs: number | string = 1;
                if (obj[key]["@minOccurs"]) minOccurs = parseInt(obj[key]["@minOccurs"], 10);
                if (obj[key]["@maxOccurs"]) maxOccurs = obj[key]["@maxOccurs"];
                if (maxOccurs == 'unbounded') maxOccurs = Number.MAX_VALUE;
                ...
                if (maxOccurs > 1) {
                    ...
                    for (let prop in obj.properties) {
                        newTD.items = obj.properties[prop];
                        obj.properties[prop] = newTD;
                    }                    
                }                
    ...
    export class XsdParser {
        ...
        parseSchema(schema: any, prefix?: string) {         
            if (prefix && prefix.indexOf(":") < 0) prefix = prefix + ":";        
            this.type = this.getJsonSchema(schema, "xmla", undefined, undefined, prefix);            
        }

        public getType(type: any, prefix?: string): any {
            ...
            if (type) {
                if (type.$ref) {
                    if (type.$ref.lastIndexOf("#/", 0) === 0) {
                        return deepFind(this.type, type.$ref.substring(2));
                    }
                } else if (typeof type === "string") {                                        
                    if (prefix && prefix.indexOf(":") <= 0) prefix = prefix + ":";
                    xsPrefix = prefix || xsPrefix;
                    var resultType = this.type.definitions[type] || mapType(type);                    
                    return resultType;
                }
                return type;
            }
        ...
        private getJsonSchema(src, title: string, outputAttrPrefix: string="@", laxURIs: boolean=false, newXsPrefix?: string) { // TODO convert to options parameter
            ...
            xsPrefix = this.getPrefix(src) || xsPrefix;
        ...
        public getPrefix(src: object, schema: string = "http://www.w3.org/2001/XMLSchema") {
            for (let p in src) {
                if (p.indexOf(':') >= 0) {
                    let pp = p.split(':')[0];
                    if (src[p]["@xmlns:" + pp] === schema) {
                        return pp + ':';
                    }
                }
            }
